---
name: metodologia-parametros-agronomicos
description: Metodología de investigación y cálculo de parámetros agronómicos (VPD, FAE, fotoperiodo, setpoints para Plantae y Fungi) al crear o editar perfiles de cultivo en la Enciclopedia de AgriEdge OS.
---

# 🌿 Metodología de Investigación de Parámetros Agronómicos (Crop Steering)

Este skill define el estándar que Antigravity y sus subagentes deben aplicar al investigar, calcular y configurar perfiles de cultivo en la Enciclopedia de AgriEdge OS.
...
- **Límites Críticos:** Si la literatura no lo especifica explícitamente, calcula automáticamente:
  - Temperatura crítica: `ideal_min - 5°C` e `ideal_max + 5°C`.
  - Humedad crítica: `ideal_min - 15%`.
  - CO2 crítico: `ideal_max + 50%`.

---

## 4. Estructura de Salida en TypeScript
Los perfiles deben implementarse respetando la interfaz `DeviceCropProfile` de [cultivo.ts](file:///C:/Users/lagos/PROYECTOS/ESP32Proyecto%20-%20Industrial/monitor-iot-backend/proyecto-iot-code-workspace/frontend_react/src/types/cultivo.ts).
